...
import { SWAPI_People } from "../types";
...
const PeoplePage = () => {
  ...
  const [searchResult, setSearchResult] = useState<SWAPI_People | null>(null);
  const [searchParams, setSearchParams] = useSearchParams();

  const query = searchParams.get("search");
  const pageParam = searchParams.get("page");

  const searchSWAPIPeople = async (searchQuery: string, searchPage = 1) => {
    setError(null);
    setLoading(true);
    setSearchResult(null);

    try {
      const res = await SWAPI.searchPeople(searchQuery, searchPage);
  ...
  useEffect(() => {
    searchSWAPIPeople(query ? query : "", Number(pageParam));
  }, [query, pageParam]);
